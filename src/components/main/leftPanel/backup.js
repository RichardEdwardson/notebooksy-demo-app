...
import { Canvas } from '../../utilities/polySelector';
...
export default function LeftPanel() {
    ...
    useEffect(() => {
        canvasRef.current.loadImage("./9.jpeg")
        const rect = canvasRef.current.getRect()
        const dpr = window.devicePixelRatio || 1
        polyRef.current.width = rect.width * dpr
        polyRef.current.height = rect.height * dpr
        const ctx = polyRef.current.getContext('2d')
        drawShape(ctx)
    }, [])

    const shapes = []
    shapes.push({ x: 300, y: 10, width: 100, height: 100 })

    const drawShape = (ctx) => {
        ctx.clearRect(0, 0, 591 * 2, 716 * 2) // Not accurate
    ...
    const mouseDown = e => {
        e.preventDefault()
        const { canvasX: startX, canvasY: startY } = getPosition(e)
        for (let shape of shapes) {
            if (isOnShape(startX, startY, shape)) {
                console.log("yes!")
            }
        }
    ...
    const getPosition = e => {
        ...
        const viewX = e.clientX - rect.x
        const viewY = e.clientY - rect.y
    ...
    return (
        <>
            <div className="flex flex-col divide-y divide-gray-200 overflow-hidden rounded-lg bg-white shadow mx-5 my-5 grow">
                ...
                <div className="px-4 py-5 sm:p-6 flex-1 flex flex-col overflow-auto">
                    <div className="relative flex-1 border-black border-2 h-1/2 w-full">
                        <Canvas
                        ...
                        <canvas
                            ...
                            onMouseMove={e => {
                                const { canvasX: x, canvasY: y } = getPosition(e)
                                const info = `Pos: (${x}, ${y})`
                                setEventInfo(info)
                            }}
                            onMouseOut={e => {
                        ...
                        />
                        {(eventInfo != null) && (
                            <div className="top-0 left-0 h-fit w-32 bg-slate-500 opacity-25 absolute z-0">
                                <h1>{eventInfo}</h1>
                            </div>
                        )}
                    </div>
...
const Main = () => {
    ...
    useEffect(() => {
        canvasRef.current.loadImage("./logo192.png")
...
<>
    <div
        ref={containerRef}
        className="overflow-scroll w-fit mx-auto mt-1">
        <Notepad
            receive={noteImage}
            width={width}
            height={height}
...
export default function BlackboardLocator({ className, data, inputImage }) {
    ...
    useEffect(() => {
        const observer = new ResizeObserver(entries => {
            ...
            const { width: imageWidth, height: imageHeight } = canvasRef.current
